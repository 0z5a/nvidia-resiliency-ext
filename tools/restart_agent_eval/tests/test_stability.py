...
class DecisionStabilityTest(unittest.TestCase):
    ...
    def _write_run(
        ...
        *,
        ...
        tool_calls: int = 0,
        analysis_status: str = "primary_identified",
        unknown_primary_recovery: bool = False,
        identity_kind: str = "root",
        root_fingerprint: str | None = "root-a",
        ...
        if l1_usable and analysis_status == "primary_identified" and unknown_primary_recovery:
            assessment = recovery_assessment(
                failure_domain="unknown",
                failure_domain_status="unknown",
                retry_outlook="unknown",
                retry_outlook_status="unknown",
                failure_domain_confidence=87,
                retry_outlook_confidence=92,
                rationale="Primary identified; recovery remains unknown.",
            )
        elif l1_usable and analysis_status == "primary_identified":
            assessment = recovery_assessment(
            ...
            assessment = recovery_assessment(
                ...
                rationale="No substantive claim.",
            )
        else:
            assessment = {}
        transcript = [
            ...
            json.dumps(
                {
                    ...
                    "analysis_result": {
                        ...
                        "primary_failure": (
                            {
                                "failure_class": "checkpoint_read",
                                "line": 12,
                            }
                            if analysis_status == "primary_identified"
                            else None
                        ),
                        "l1_assessment": {
        ...
        panel = {
            ...
            "rows": [
                ...
                },
                {
                    ...
                    "current_identity_kind": identity_kind,
                    "current_root_fingerprint": root_fingerprint,
                    "current_observation_fingerprint": observation_fingerprint,
                    "selected_observation_class": (
                        "process_terminated" if identity_kind == "observation_only" else None
                    ),
                    "selected_observation_line": (
                        44 if identity_kind == "observation_only" else None
                    ),
    ...
    def test_observation_only_identity_stability_is_separate_from_root_stability(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runs = [
                self._write_run(
        ...
        identity = summary["cohorts"][0]["primary_and_identity_stability"]
        self.assertEqual(identity["identity_kind"]["distribution"], {"observation_only": 3})
        self.assertEqual(identity["root_fingerprint"]["count"], 0)
        self.assertEqual(identity["observation_fingerprint"]["modal_agreement"], 1.0)

    def test_non_primary_confidence_placeholders_are_excluded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runs = [
                self._write_run(
        ...
        confidence = summary["cohorts"][0]["semantic_stability"]
        self.assertEqual(confidence["failure_domain_confidence"]["count"], 0)
        self.assertEqual(confidence["retry_outlook_confidence"]["count"], 0)

    def test_unknown_primary_claim_confidence_is_excluded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runs = [
                self._write_run(
