...
4. **Deterministic result before model enrichment**: L0A freezes shared Decision Evidence;
...
- Decision Evidence is built once from L0A and is byte-identical at the branch
  point. The deterministic branch does not rescan the log or wait for L1.
- The enriched branch adds model semantics, grounding, identity, and an advisory audit to the same canonical
  ...
  deadline; otherwise the deterministic recommendation remains available.
...
L0 also owns the deterministic recommendation root fingerprint. Its runtime KPI is
...
- **Route outcome:** whether the complete route produced a usable enriched
  result within the measured decision window, or required NVRx fallback.
...
| Enriched track readiness | Whether L2 independently produced primary/root and observation keys, their sources, and whether L3 can consume each | current |
| L0 relationship | Whether the L2 key equals the L0 deterministic key; diagnostic when primaries differ | current |
...
history, takes one immutable in-memory view before deterministic/model branches run, and
upserts the current cycle entry afterward. That entry preserves shared progress,
deterministic facts, and route-keyed primary/observation tracks. History is enabled by
...
1. Preserve deterministic evidence and independently ground primary and
   observation tracks for each route.
...
5. Compare only distinct earlier cycles with exact job, route, track kind, and
   fingerprint equality; never substitute another track.
...
8. Preserve exact failure/data/artifact matches and consecutive same-root or
   same-observation no-advance counts for L4. L3 does not choose a track or the
   required count.
...
The product exposes a root fingerprint plus optional exact affected entity, and
a separately typed observation fingerprint. L0 owns deterministic identity;
L2 owns each route's grounded primary and observation identities. The harness
measures track isolation, cross-route agreement, and corpus false merge/split
behavior without treating route agreement as additional attempts.

| Metric | What it measures | Availability |
| --- | --- | --- |
| History availability | Whether eligible prior attempts were available | current |
| Track isolation | Deterministic, route-primary, and route-observation facts compare only with like-kind history | current/test |
| Cycle deduplication | Multiple agreeing routes do not multiply one prior cycle | current/test |
...
| Path-selection accuracy | Primary, observation, deterministic, or none is chosen with the specified precedence and matching history | current/test; gold pending |
...
| Result quality | Normal, degraded, late, or unusable result classification | current |
...
2. **Per-attempt user payload:** `_initial_user_message(...)` from `llm.py`. It
   is rebuilt for each log as a facts-only JSON object with exactly four
   top-level sections: `response_schema`, `decision_evidence`,
   `attempt_execution_context`, and `evidence_bundle`.
...
Context quality and latency are addressed by incremental terminal ingestion
during bounded log drain. Progressive pre-end ingestion is an optional
optimization when target-scale measurements justify it.

**Content**

- NVRx cycle start registers the attempt. L0A tails and precomputes before end
  only when the Restart Agent progressive optimization is explicitly enabled.
...
- The default production latency measurement begins at terminal submission.
  An enabled progressive profile additionally measures from `progressive_end`.
...
- Evidence convergence means the available bytes required to finalize L0A have
  ...
  deterministic L3/L4 recommendation, and launch the L0B/L1 enriched path.
- Progressive mode may shift precomputable L0A work earlier; it does not remove
  log-convergence latency. Measure workload end, evidence convergence, and
  decision readiness as distinct times for either schedule.
...
L0A may retain many normalized occurrence groups and windows for trace/debug, deterministic,
...
- model-enriched versus deterministic-deterministic contribution;
...
4. `evidence_bundle`: the bounded model-visible evidence selected by L0B.

Immutable restart guarantees are stated once in the static system prompt, not
repeated as per-attempt data.
