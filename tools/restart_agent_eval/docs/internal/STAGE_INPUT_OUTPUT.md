...
L0A Complete Evidence Assembly -> immutable complete structured bundle
            ...
            compact canonical decision inputs
             ...
             |                                         |
             v                                         v
Deterministic L3 -> L4             L0B Initial Model Evidence View
...
the L0A scan; it exists to prevent the deterministic and model branches from
independently selecting the same policy-relevant facts.

The deterministic and enriched branches therefore share the same deterministic
facts. As soon as Decision Evidence is frozen, deterministic L3/L4 starts without
...
L0B/L1/L2 before its own L3/L4 evaluation. The earlier deterministic result remains usable
...
one immutable in-memory prior-attempt view before either branch starts, then
upserts the current cycle entry after analysis. Configuration parsing and
...
This table describes the implemented contract.
...
| Decision Evidence shared artifact | Canonical L0A facts required by downstream decision logic | Compact deterministic primary or selected observation, identity, progress, recovery, and placement facts | Deterministic L3/L4 and model-visible L0B projection |
...
| L1 Semantic Analysis | L0B view, prompt/schema, model profile, and optional tools backed by L0A/raw logs | Raw primary plus independent non-primary observed-failure assessment; no action | L2 |
| L2 Grounding, Identity, And Audit | Raw L1 output, L0A bundle, L0B view, tool evidence, source log | Independently grounded route-primary/root and route-observation `AttemptFailureFacts`, mechanical reference repairs, and advisory findings | L3 tracks and observability; L4 receives unchanged L1 semantics |
| L3 History Enrichment | `CycleFailureEntry`, current progress, job/cycle identity, immutable runtime-selected prior attempts | Shared job-progress comparison plus like-kind deterministic, route-primary, and route-observation histories | L4 |
| L4 Policy Decision | Preserved failure tracks, matching L3 histories, unchanged applicable L1 assessment, policy profile | Selected evidence path, retry rule/history scope, allowed/count/exhausted state, decision basis, action, and provenance | Candidate selector/NVRx |
| Candidate selector | Deterministic and enriched candidates plus readiness time | Best candidate available before the caller deadline | NVRx |
...
| `analysis_mode` | no | `terminal`, `progressive_start`, or `progressive_end`; defaults to `terminal`. Attrsvc terminal execution is the production default; progressive pre-end L0A polling is an implemented opt-in. |
...
AnalysisResult
  ...
  observed_failures: grounded visible failure surfaces when no primary is supportable
  selected_observed_failure: optional root-independent observation selected from that list
  l1_assessment: exact parsed L1 semantic output, unchanged by later stages
  l2_grounding: grounded primary/related evidence, identity, adjustments, and findings
  retry_policy: L4 rule, allowed retries, matched priors, and exhaustion state
  secondary_failures, cascades, evidence_coverage
  result_provenance:
    evidence_source
    model_contribution
...
first_downstream_cascade:
  line: 1030
  failure_class: process_teardown
...
The selection is built once from L0A and shared by both the deterministic and model
...
- exact reuse by deterministic and model branches;
...
- immutable restart guarantees from the static system prompt: unchanged
  workload, recreated process state, normal restart delay, and hardware
  allocation or mutable external-service state that may change.
...
- **Semantic quality:** semantic-primary accuracy against accepted gold lines
  ...
  over substantive `primary_identified` claims. Unknown and non-primary
  confidence placeholders are retained but excluded from calibration.
...
- **Route outcome:** model-enriched versus deterministic contribution, final result
...
- unusable final structure means L2 is not run and L3/L4 use the deterministic path;
...
L0 already owns the deterministic-path root fingerprint. A model may propose a
...
### Functional Outputs: `L2Result`, Failure Tracks, And Observational Audit

```text
citation audit
  raw exact, rendered exact, abbreviated exact, nearby resolved, ungrounded
field findings
  field, code, message, severity, observational_only=true
ignored prohibited fields
audited related-failure roles
mechanical reference repairs with before/after/reason
stable primary identity anchor and deterministic root fingerprint
independent selected-observation fingerprint
route primary/observation AttemptFailureFacts for the L3 boundary
experimental family/concrete/client identities
grounded primary and related failures
grounded observed failures and selected observation
recovery-support observations and unapplied suggestions
```

Raw L1 remains unchanged. An exact source line or unique bounded quote match is
required before L2 can create a root or observation history identity. If that grounding is not
available, current-cycle L1 semantics remain observable and usable, but the
affected track is unavailable to L3 recurrence. L2 may resolve an evidence line. When a semantic
field appears weak, L2 records an independent suggestion but cannot apply it.
An observation may reference an undefined canonical evidence ID while still
providing a valid model-visible source line. L2 records
`dangling_evidence_reference`, excludes the unresolved ID from the grounded
citation projection, and may ground the observation from that direct line. The
finding remains observational and does not make the route degraded.
L2 findings do not change route usability, L1 scoring, L3 identity, L4 policy,
or action. L4 receives the exact typed L1 recovery assessment once the selected
primary or selected observation is mechanically grounded. Observation-only
recovery assessment remains restricted to root-independent policy.
...
Findings may retain severity for review prioritization, but all carry the same
authority: observational only. They do not degrade the result, alter policy
inputs, or enter history.
...
- mechanical-reference-repair and recovery-support-observation counts;
...
- identity kind plus enriched root- or observation-fingerprint
  owner/value/availability/source/history-readiness,
  relation to the L0 deterministic fingerprint, and derivation latency;
- gold fingerprint accuracy plus corpus false-merge and false-split rates.

L2 owns enriched primary and observation fingerprints. L0 owns the
deterministic-path fingerprint. Each is normalized to the same
current-log-only `AttemptFailureFacts` contract. L3 receives all tracks plus
prior `attempt_history` and does not rewrite or select a fingerprint.

### Degraded Behavior

- structurally unusable L1 means `audit_status=not_run`;
- an ungrounded primary makes the enriched branch unavailable;
- audit findings remain visible without changing route quality or NVRx
  eligibility.
...
- the current `CycleFailureEntry`: deterministic facts plus route-keyed primary
  and observation facts;
- `job_id` and current `cycle_id`;
- compact prior `attempt_history` from distinct cycles, selected by
  `RestartAgentRuntime` before deterministic and enriched branches run. This is an
  immutable in-memory view, not a serialized snapshot artifact.

Each `AttemptRecord` includes the stable root identity, optional exact affected
entity, and a deterministic `progress` summary. The summary records `observed`, `not_observed`, or
...
### Functional Output: `CycleHistoryComparison`

```text
shared job-progress history
deterministic HistorySummary
routes[route_id].primary HistorySummary
routes[route_id].observation HistorySummary
```

Each non-null `HistorySummary` retains same-root or same-observation counts,
affected-entity relations where applicable, per-cycle progress deltas, observer
relations, and consecutive no-observed-advance streaks.

Root comparison requires exact `job_id`, distinct earlier cycles, the same
track kind and route, and the same grounded root fingerprint. Observation-only
comparison applies the same filtering to a separate observation fingerprint
and never enables root or entity ledgers. Missing same-route history remains
unavailable rather than falling back to deterministic facts. L3 additionally compares typed affected
entities as `same`, `different`, or `unknown`. L3 compares failure iteration only with failure
...
L4 chooses the evidence path and then uses only that path's root-only,
root-plus-entity, or observation history. The absolute summary remains
separately visible.
...
- deterministic, primary, and observation track availability by route;
- cross-kind isolation and missing-route-fallback accuracy;
- per-cycle deduplication when multiple routes agree;
...
- affected-entity extraction and relation accuracy;
- root-only and root-plus-entity consecutive no-observed-advance accuracy;
- observation-only recurrence and no-observed-advance accuracy;
...
- unchanged current-cycle L1 primary and recovery assessment, or the
  conservative Decision Evidence deterministic recommendation;
- L3 `HistorySummary`;
...
selects a base rule and effective retry policy, evaluates concurrent root and
same-job ledgers, and emits the action with a deterministic decision basis.
...
| `concrete_confirmation_retry` | Root fingerprint plus exact non-null affected entity | configurable, default `1`, exact root and entity |
| `workload_confirmation_retry` | Grounded workload domain without full identity | configurable, default `1`, root only |
| `general_retry` | Unknown, degraded, or otherwise unqualified current-attempt semantics | configurable, default `2`, root only |

A matching `rejected_iteration_retry_then_skip` policy context preserves the
base rule but supplies the effective rule and a default two-retry ledger. Gold
labels may score these independently with `accepted_rules` and
`accepted_effective_rules`.

Only distinct prior cycles matching a ledger's scope and showing no observed
advance count toward its exhaustion. Root-independent same-job guards separately
bound repeated no-advance and unknown-progress attempts.

### Functional Outputs: `L4DecisionOutput` And `AnalysisResult`

```text
retry_policy
  policy_version, base_rule, effective_policy, applied_policy_context
  retry_budget_exhausted, exhausted_by
  general_root_ceiling:
    root-only allowed retries, matching prior attempts, observed advance,
    exhaustion, applicability
  selected_policy_ledger (optional):
    selected rule, root-only or root-and-entity scope, allowed retries,
    matching prior attempts, observed advance, exhaustion
  job_no_progress_guard, job_unknown_progress_guard:
    same-job allowed retries, matching prior attempts, observed advance,
    exhaustion
  current_evidence_qualified, current_affected_entity
decision_basis
decision: STOP | RESTART
...
- absent primary produces unusable quality and NVRx default handling;
...
## Terminal-First Lifecycle And Optional Progressive Overlap

Terminal execution begins incremental L0A work at workload end while observing
bounded shared-log drain. Optional progressive execution can move some of that
work earlier, but neither schedule equates workload end with complete
shared-log evidence:

```text
terminal default:
cycle start        workload running        terminal submit       log converges
    | register only ---------------------------->|-- L0A + drain overlap -->|
...
- By default, `progressive_start` registers the attempt without pre-end polling.
- With explicit progressive enablement, `progressive_start` begins L0A tailing
...
- Freezing Decision Evidence starts deterministic L3/L4 and the L0B/L1 enriched path
  from the same canonical facts. Candidate selection remains deadline-aware.

Terminal replay measures terminal request, convergence, L0A-ready,
deterministic-ready, and enriched-ready times. Progressive replay reports the same
milestones plus work completed before cycle end; it is required only to qualify
an explicitly enabled progressive profile.

## Candidate Selection And Deadline

| Candidate | Inputs | Ready condition | Purpose |
| --- | --- | --- | --- |
| `deterministic` | Decision Evidence + L3 + L4 | As soon as deterministic assembly/history/policy finish | Deadline-safe recommendation when L1 is late or unavailable. |
...
enriched result. Attrsvc publishes the deterministic recommendation independently and
implements both terminal drain and opt-in progressive start/end state.
