...
product CLI: collect_all
        |
        +--> shared L0A --> DecisionEvidence --> deterministic L3 --> L4
        |
        +--> shared L0B --> parallel independent model routes
                            --> L1 --> L2 --> L3 --> L4
        |
        v
...
Panel execution explicitly enables enrichment in the generated product
configuration. This is a harness choice needed to compare routes; it does not
alter the product's deterministic-only default.
...
Terminal replay is implemented and measures the default production schedule
from terminal submission to each usable candidate. Progressive replay remains a
separate qualification mode for explicitly enabled pre-end polling profiles; it
measures from `progressive_end` and reports work shifted before that signal.
...
result, and deep trace. `review.json` stores harness-owned KPIs, scores, concerns,
and compact comparison fields; it does not copy complete product stage outputs.
The generated `review.md` renders compact semantic and evaluation summaries and
links to `result.json` for canonical L1, L2, and final policy output.
...
Review reporting keeps L1 RCA, L1 recovery semantics, L2 primary/root and
observation identities, per-track L3 history, L4 path selection, final cascade
grouping, and L4 policy/action as independent outcomes. L2 identity gold
describes track kind, canonical anchor, operation, mechanism, and expected
cross-route cardinality; it does not freeze an implementation hash. Replayed L0
artifacts are labeled as replay rather than counted as zero-cost assembly.

`ReviewContext` preserves the L0 deterministic view,
unchanged L1 primary and observed-failure views, independently grounded L2
primary/observation tracks, per-track L3 summaries, and the L4-selected path.
Observation fingerprints have their own agreement and gold KPIs and cannot
satisfy, reset, or fail a root fingerprint check. When the product does not
publish a track, the harness marks it unavailable; it does not infer a
replacement from the final public result.
...
primary, identity-kind, root-fingerprint, and observation-fingerprint
agreement, tool-path variability, endpoint health,
...
5. Score deterministic, enriched-primary, and enriched-observation history and
   L4 path selection independently.
...
not rewrite L1 semantics. L3 reports root-only and root-plus-entity recurrence
without selecting a threshold, and L4 owns policy/action and match scope.
...
4. route outcome, including model-enriched versus deterministic contribution,
