...
- Model-panel profiles MUST explicitly set `enrichment.enabled=true`. The
  product default remains deterministic-only and MUST NOT be silently changed
  by a missing harness setting.
...
calibration MUST be reported only over a corpus. Calibration MUST include only
substantive claims from `primary_identified` results. Confidence attached to an
`unknown` claim or the canonical `confidence=1` non-primary placeholder MUST
remain available in raw artifacts but MUST NOT enter calibration or
confidence-distribution KPIs. Human summaries MUST display its confidence as
`N/A`.
...
retry_policy_expectation:
  accepted_rules: [workload_confirmation_retry]
  accepted_effective_rules: [rejected_iteration_retry_then_skip]
  allowed_retries: 1
  retry_budget_exhausted: false
```

`accepted_rules` applies to the base rule;
`accepted_effective_rules` applies to the effective rule after policy-context
resolution. When both are present, both MUST match. Rule selection, effective
allowed retries, exhaustion, decision basis, and action MUST be evaluated
independently when labeled. Effective allowed retries means
`effective_policy.allowed_retries`. Review artifacts
MUST retain both ledger counts and scopes even though this gold field scores
their policy-level effective allowance. A correct action MUST NOT hide an
incorrect rule or retry count.
...
When gold expects no primary but one selected terminal observation, L0A quality
MUST instead score primary absence, selected-observation accuracy, and
observation-fingerprint accuracy without treating that fingerprint as a root.
...
attempts, retries, provider timeouts, HTTP/provider failures, and failed-attempt
time. Analysis-deadline exhaustion MUST remain a route-budget outcome rather
than an endpoint fault.

### L2

Quality SHOULD cover citation/reference correctness, audit correctness, and
independent primary-root and observation fingerprint correctness/merge/split
behavior. Runtime metrics MUST show per-track grounding status, citation
outcomes, observational findings by severity, identity readiness, and latency.
L2 disagreement MUST NOT be reported as an L1 rewrite.
L2 findings MUST NOT affect route quality, NVRx eligibility, policy, or action.
Grounded observation identity MUST be reported separately from grounded root
identity. Missing root fingerprints are not defects for routes explicitly
labeled `identity_kind=observation_only`.

### L3

Quality SHOULD cover exact job/root filtering, affected-entity extraction and
matching, progress relations, matching prior counts, root-only versus
root-plus-entity streaks, track isolation, missing-route-history behavior, and
per-cycle deduplication across agreeing routes. Unknown progress MUST remain distinct from no
observed advance.
Observation-only recurrence and its no-advance streak MUST remain separate
from root recurrence; same-job progress guards remain independently visible.
...
Reports MUST expose the selected evidence path, route, selection reason,
matching history scope, affected entity, and higher-precedence track
availability.

The deterministic recommendation L4 result and every available L1-enriched L4 result
...
`not_available`, not be counted as agreement with the deterministic result.
...
- Usable L1 rate, final-action agreement and sequential flips, exact
  action-driving L1 field agreement, primary and identity-kind agreement,
  separate root- and observation-fingerprint agreement, tool-path variability,
  endpoint reliability, latency, and tokens MUST be reported separately.
...
- Terminal request-to-result MUST be the default production qualification
  metric.
- An explicitly enabled progressive profile MUST additionally measure from
  `progressive_end` to usable result and separately report work shifted before
  that signal. The two schedules MUST remain separately labeled.
...
append-only lifecycle event stream, the deterministic recommendation when available,
