...
def _panel_diagnostics_markdown(panel: dict[str, Any]) -> str:
    ...
    _append_deterministic_and_decision_evidence(panel, lines)
...
def _append_deterministic_and_decision_evidence(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            "",
            "## Deterministic Path Inputs",
            "",
            "This is the shared, model-independent L0 `DecisionEvidence` used by the "
            "deterministic path independent of L1. It is shown once because L0 "
    ...
    if decision_evidence:
        deterministic_primary = _dict(decision_evidence.get("deterministic_primary_candidate"))
        ...
        deterministic_fingerprint = deterministic_primary.get("root_fingerprint")
        ...
        lines.extend(
            [
                ...
                "",
                "| primary | outcome | phase | causal role |",
                "|---|---|---|---|",
                "| "
                f"{_md(deterministic_primary.get('failure_class'))}@"
                f"{_md(deterministic_primary.get('line'))} | "
                f"{_md(deterministic_primary.get('fault_outcome'))} | "
                f"{_md(deterministic_primary.get('phase'))} | "
                f"{_md(deterministic_primary.get('causal_role'))} |",
                "",
                "### L0 Fallback Root-Fingerprint KPI",
                "",
                "L0 owns the deterministic fingerprint. L3 consumes this key "
                ...
                "| "
                "L0 | "
                f"{_yes_no(bool(deterministic_fingerprint))} | "
                f"{_yes_no(bool(deterministic_fingerprint))} | "
                f"{_md(deterministic_primary.get('root_fingerprint_source'))} | "
                ...
                "",
                "```text",
                str(deterministic_fingerprint or "<none>"),
                "```",
            ]
        )
        if canonical_fingerprint != deterministic_fingerprint:
            lines.extend(
                [
                    "",
...
def _append_root_fingerprints(panel: dict[str, Any], lines: list[str]) -> None:
    if any(row.get("l2_root_fingerprint") for row in panel["rows"]):
        ...
        lines.extend(
            [
                ...
                f"available_models=`{entity_agreement.get('available_models')}`, "
                f"unique_fingerprints=`{entity_agreement.get('unique_fingerprints')}`, "
                f"disagreement_reason=`{entity_agreement.get('disagreement_reason')}`",
                "",
                "| target | available | history ready | matches L0 | identity anchor | source | root | affected entity |",
                "|---|---|---|---|---:|---|---|---|",
            ]
        )
        for row in panel["rows"]:
            lines.append(
                ...
                f"{_md(_affected_entity_label(row.get('l2_affected_entity')))} |"
            )
        history_roots = {
    ...
    if any(row.get("l2_observation_fingerprint") for row in panel["rows"]):
        agreement = _dict(panel.get("l2_observation_fingerprint_agreement"))
        lines.extend(
            [
                "",
                "## L2 Observation-Only Identity KPI",
                "",
                "Observation fingerprints compare visible terminal failure surfaces. They remain separate from root and affected-entity policy ledgers.",
                "",
                "- agreement: "
                f"status=`{agreement.get('status')}`, "
                f"available_models=`{agreement.get('available_models')}`, "
                f"unique_fingerprints=`{agreement.get('unique_fingerprints')}`, "
                f"disagreement_reason=`{agreement.get('disagreement_reason')}`",
                "",
                "| target | identity kind | selected line | history ready | source | observation |",
                "|---|---|---:|---|---|---|",
            ]
        )
        for row in panel["rows"]:
            lines.append(
                "| "
                f"{_md(row.get('target'))} | "
                f"{_md(row.get('l2_identity_kind'))} | "
                f"{_md(row.get('l2_selected_observation_line'))} | "
                f"{_yes_no(row.get('l2_history_identity_ready'))} | "
                f"{_md(row.get('l2_observation_fingerprint_source'))} | "
                f"{_md(_short_identity(row.get('l2_observation_fingerprint')))} |"
            )
...
def _append_gold_results(panel: dict[str, Any], lines: list[str]) -> None:
    if any(row.get("gold_case_id") for row in panel["rows"]):
        ...
        if gold_l0a_row is not None:
            lines.extend(
                [
                    ...
                    (
                        "- deterministic root fingerprint accuracy: "
                        f"`{gold_l0a_row.get('gold_l0a_root_fingerprint_accuracy')}`"
                    ),
                    (
                        "- selected observation: "
                        f"accuracy=`{gold_l0a_row.get('gold_l0a_selected_observation_accuracy')}`, "
                        "fingerprint_accuracy="
                        f"`{gold_l0a_row.get('gold_l0a_observation_fingerprint_accuracy')}`, "
                        f"primary_absent=`{gold_l0a_row.get('gold_l0a_primary_absence_correct')}`"
                    ),
                    (
                        "- markers: "
        ...
        lines.extend(
            [
                "| target | L2 audit | L2 history identity | primary-grounding effect | L4 root | L4 policy | L4 action | L4 overall |",
                "|---|---|---|---|---|---|---|---|",
            ]
        )
        for row in panel["rows"]:
            lines.append(
                ...
                f"{_md(row.get('gold_l2_primary_grounding_effect'))} | "
...
def _append_l2_diagnostics(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            "| target | grounding | method | audit | ran | primary | recovery audited | related | domain/retry/unresolved support | citations raw/rendered/abbrev/nearby/U | observational findings | severity |",
            "|---|---|---|---|---|---|---|---:|---|---|---:|---|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_yes_no(row.get('l2_recovery_assessment_audited'))} | "
...
def _append_l1_semantic_assessment(panel: dict[str, Any], lines: list[str]) -> None:
    lines.extend(["", "## L1 Restart Guarantees", ""])
    lines.append(
        "- Static product guarantees: the workload remains unchanged, process state is "
        "recreated, normal restart delay applies, hardware allocation may change, and "
        "external-service state may change."
    )
...
def _append_l3_history(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "| target | source | identity kind | history identity ready | selected root | selected observation | affected entity |",
            "|---|---|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_md(_short_identity(row.get('current_observation_fingerprint')))} | "
            f"{_md(_affected_entity_label(row.get('current_affected_entity')))} |"
        )
    lines.extend(["", "### Prior-Attempt Comparison", ""])
    lines.extend(
        [
            "| target | available | same job/root/observation | advanced | no advance | unknown | same failure | entity S/D/U | root/entity/observation streak |",
            "|---|---|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_md(row.get('l3_matching_observation_attempts'))} | "
            ...
            f"{_md(row.get('l3_same_entity_attempts'))}/"
            f"{_md(row.get('l3_different_entity_attempts'))}/"
            f"{_md(row.get('l3_unknown_entity_attempts'))} | "
...
def _append_l4_policy(panel: dict[str, Any], lines: list[str]) -> None:
    ...
    lines.extend(
        [
            "| target | decision | basis | base -> effective | root ledger | selected ledger | job guards | exhausted by | current evidence qualified | quality | NVRx use |",
            "|---|---|---|---|---:|---|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_diagnostic_policy_rule(row)} | "
            f"{_diagnostic_ledger(row, 'l4_general_root')} | "
            f"{_diagnostic_selected_ledger(row)} | "
            f"{_diagnostic_job_guards(row)} | "
    ...
    lines.extend(
        [
            "| target | domain | domain status | retry outlook unchanged | retry status | observed advance | affected entity |",
            "|---|---|---|---|---|---|---|",
        ]
    )
    for row in panel["rows"]:
        lines.append(
            ...
            f"{_md(_affected_entity_label(row.get('l4_current_affected_entity')))} |"
        )
...
def _diagnostic_ledger(row: dict[str, Any], prefix: str) -> str:
