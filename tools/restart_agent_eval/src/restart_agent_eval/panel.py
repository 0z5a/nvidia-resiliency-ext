...
def _build_panel_payload(context: PanelInput) -> dict[str, Any]:
    ...
    shared_decision_evidence_path = index.get("shared_decision_evidence")
    ...
    ) = _shared_decision_evidence(
        summaries,
        shared_path=(
            Path(str(shared_decision_evidence_path)) if shared_decision_evidence_path else None
        ),
    )
    ...
    root_rows = [row for row in model_rows if row.get("l2_identity_kind") == "root"]
    observation_rows = [
        row for row in model_rows if row.get("l2_identity_kind") == "observation_only"
    ]
    ...
    affected_entity_agreement = _fingerprint_agreement(
        root_rows,
        fingerprint_field="l2_affected_entity_fingerprint",
        primary_line_field="l2_grounded_primary_line",
        require_multiple=True,
    )
    observation_agreement = _fingerprint_agreement(
        observation_rows,
        fingerprint_field="l2_observation_fingerprint",
        primary_line_field="l2_selected_observation_line",
        require_multiple=True,
    )
    l2_identity_kind_counts = Counter(
        str(row.get("l2_identity_kind") or "none") for row in model_rows
    )
    history_identity_kind_counts = Counter(
        str(row.get("current_identity_kind") or "none") for row in model_rows
    )
    resolved_identity_kind_counts = dict(sorted(history_identity_kind_counts.items()))
    ...
    expected_identity_kinds = {
        str(row["gold_l2_expected_identity_kind"])
        for row in model_rows
        if row.get("gold_l2_expected_identity_kind") is not None
    }
    expected_identity_kind = (
        next(iter(expected_identity_kinds)) if len(expected_identity_kinds) == 1 else None
    )
    scored_identity_agreement = (
        observation_agreement if expected_identity_kind == "observation_only" else root_agreement
    )
    root_agreement["gold_expected_identity_count"] = expected_identity_count
    root_agreement["gold_stability_correct"] = (
        scored_identity_agreement["unique_fingerprints"] == expected_identity_count
        and scored_identity_agreement["available_models"]
        == (
    ...
    root_agreement["gold_expected_identity_kind"] = expected_identity_kind
        ...
        concerns.append(
            {
                ...
                "summary": (
                    ...
                    f"available_models={root_agreement['available_models']}/{len(root_rows)} "
        ...
        concerns.append(
            {
                ...
                "summary": (
                    ...
                    f"expected_identity_kind={expected_identity_kind} "
                    f"observed_unique_identities="
                    f"{scored_identity_agreement['unique_fingerprints']}"
                ),
            }
        )
    if affected_entity_agreement["status"] == "unstable" or (
        affected_entity_agreement["available_models"] > 0
        and affected_entity_agreement["disagreement_reason"] == "missing_fingerprints"
    ):
        concerns.append(
            {
                "target": "panel",
                "category": "affected_entity_stability",
                "summary": (
                    f"status={affected_entity_agreement['status']} "
                    f"available_models={affected_entity_agreement['available_models']}/"
                    f"{len(root_rows)} "
                    f"unique_fingerprints="
                    f"{affected_entity_agreement['unique_fingerprints']} "
                    f"reason={affected_entity_agreement['disagreement_reason']}"
                ),
            }
        )
    if len(history_identity_kind_counts) > 1:
        concerns.append(
            {
                "target": "panel",
                "category": "history_identity_kind_disagreement",
                "summary": f"identity_kind_counts={resolved_identity_kind_counts}",
            }
        )
    if observation_agreement["status"] == "unstable":
        concerns.append(
            {
                "target": "panel",
                "category": "l2_observation_fingerprint_stability",
                "summary": (
                    f"status={observation_agreement['status']} "
                    f"available_models={observation_agreement['available_models']}/"
                    f"{len(observation_rows)} "
                    f"unique_fingerprints={observation_agreement['unique_fingerprints']} "
                    f"reason={observation_agreement['disagreement_reason']}"
                ),
            }
        )
    ...
    return {
        ...
        "l2_observation_fingerprint_agreement": observation_agreement,
        "l2_identity_kind_counts": dict(sorted(l2_identity_kind_counts.items())),
        "history_identity_kind_counts": resolved_identity_kind_counts,
        "affected_entity_agreement": affected_entity_agreement,
        "concerns": concerns,
        "notes": [
            (
...
def _decision_path_comparison(rows: list[dict[str, Any]]) -> dict[str, Any]:
    available_deterministic_results = [
        {
            "decision": row.get("deterministic_decision"),
            "retry_rule": row.get("deterministic_retry_rule"),
            "gold_action_correct": row.get("gold_deterministic_action_correct"),
            "gold_policy_action": row.get("gold_deterministic_policy_action"),
        }
        for row in rows
        if row.get("deterministic_available")
    ]
    serialized_deterministic_results = {
        json.dumps(item, sort_keys=True, separators=(",", ":"))
        for item in available_deterministic_results
    }
    if not available_deterministic_results:
        deterministic_status = "unavailable"
        shared_deterministic: dict[str, Any] = {}
    elif len(serialized_deterministic_results) == 1:
        deterministic_status = "consistent"
        shared_deterministic = available_deterministic_results[0]
    else:
        deterministic_status = "inconsistent"
    ...
    return {
        "deterministic_consistency": deterministic_status,
        "shared_deterministic": shared_deterministic,
...
def _enrich_concern(concern: dict[str, Any]) -> dict[str, Any]:
    ...
    if category.startswith("l0_"):
        owner = "L0 bundle"
    elif category.startswith("l2_") or category == "history_identity_kind_disagreement":
    ...
    if category in {
        ...
        "l2_observation_fingerprint_stability",
        "history_identity_kind_disagreement",
    ...
    impact = {
        ...
        "l2_observation_fingerprint_stability": "observation identity stability",
        "history_identity_kind_disagreement": "cross-route failure interpretation",
...
def _comparison_axes(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        published_model_contribution = row.get("l4_model_contribution")
        model_enriched = published_model_contribution == "attempted_used"
        deterministic_only = not model_enriched
        model_contribution = "model_enriched" if model_enriched else "deterministic_only"
        ...
        route_outcome.append(
            {
                ...
                "nvrx_use": (
                    ...
                    if deterministic_only and row.get("l4_nvrx_use") == "eligible_degraded"
                    else row.get("l4_nvrx_use")
                ),
                "reason": published_model_contribution or "model_contribution_unavailable",
...
def _shared_decision_evidence(
    ...
    *,
    ...
    payload_by_digest: dict[str, dict[str, Any]] = {}
    for summary in summaries:
        target = str(summary.get("target") or summary.get("run_label") or "unknown")
        evidence = _dict(summary.get("decision_evidence"))
        digest = summary.get("decision_evidence_sha256")
        if not digest and evidence:
            digest = _canonical_payload_sha256(evidence)
        if not digest:
            continue
        digest = str(digest)
        evidence_by_target[target] = {"sha256": digest}
        if evidence:
            payload_by_digest[digest] = evidence

    total_models = len(summaries)
    available_models = len(evidence_by_target)
    unique_digests = {item["sha256"] for item in evidence_by_target.values()}
    ...
    if unique_payloads == 1:
        ...
        shared = payload_by_digest.get(digest, {})
        if not shared and shared_path is not None and shared_path.is_file():
            candidate = _dict(_read_json(shared_path))
            if _canonical_payload_sha256(candidate) == digest:
                shared = candidate
...
def _canonical_payload_sha256(value: Any) -> str | None:
    if value in (None, {}, []):
        return None
...
class _PanelRowState:
    ...
    gold_deterministic_l4: dict[str, Any]
    gold_enriched_l4: dict[str, Any]
    gold_path_comparison: dict[str, Any]
    deterministic_path: dict[str, Any]
...
def _prepare_panel_row_state(summary: dict[str, Any]) -> _PanelRowState:
    ...
    observation_selection = _dict(summary.get("observation_selection_by_stage"))
    ...
    l0_selected_observation = _dict(observation_selection.get("l0_deterministic"))
    l1_selected_observation = _dict(observation_selection.get("l1_semantic"))
    l2_selected_observation = _dict(observation_selection.get("l2_grounded"))
    selected_observation = _dict(summary.get("selected_observed_failure"))
    failure_tracks = _dict(summary.get("failure_tracks"))
    cycle_history = _dict(summary.get("l3_history"))
    l4_path_selection = _dict(summary.get("l4_path_selection"))
    history_route_id = l4_path_selection.get("route_id") or summary.get("target")
    route_history = _dict(_dict(cycle_history.get("routes")).get(history_route_id))
    current_failure_facts = _dict(summary.get("history_identity_comparison")) or _dict(
        summary.get("current_failure_facts")
    )
    ...
    semantic_comparison = _dict(summary.get("semantic_comparison"))
    if semantic_comparison:
        recovery_assessment = {
            "failure_domain": _dict(semantic_comparison.get("failure_domain")),
            "retry_outlook_without_workload_change": _dict(
                semantic_comparison.get("retry_outlook_without_workload_change")
            ),
        }
        root_cause = {"status": semantic_comparison.get("root_cause_status")}
    else:
    ...
    if not l0_bundle_kpis:
        l0_bundle_kpis = _default_l0_kpis(
            summary=summary,
            tool_efficiency=tool_efficiency,
        )
    ...
    gold_deterministic_l4 = _dict(gold_score.get("deterministic_l4"))
    ...
    deterministic_path = _dict(decision_paths.get("deterministic"))
    ...
    return _PanelRowState(
        ...
        gold_l2=gold_l2,
        gold_l4=gold_l4,
        gold_deterministic_l4=gold_deterministic_l4,
...
def _panel_row_route_and_identity_fields(state: _PanelRowState) -> dict[str, Any]:
    deterministic_track = _dict(state.failure_tracks.get('deterministic'))
    primary_track = _dict(state.failure_tracks.get('primary'))
    observation_track = _dict(state.failure_tracks.get('observation'))
    deterministic_history = _dict(state.cycle_history.get('deterministic'))
    primary_history = _dict(state.route_history.get('primary'))
    observation_history = _dict(state.route_history.get('observation'))
    return {
        ...
        'failure_tracks': state.failure_tracks,
        'l3_history': state.cycle_history,
        'l4_path_selection': state.l4_path_selection,
        'deterministic_track_status': deterministic_track.get('status') or 'not_available',
        'primary_track_status': primary_track.get('status') or 'not_available',
        'observation_track_status': observation_track.get('status') or 'not_available',
        'deterministic_history_status': _history_track_status(deterministic_history),
        'primary_history_status': _history_track_status(primary_history),
        'observation_history_status': _history_track_status(observation_history),
        'l4_selected_path': state.l4_path_selection.get('path'),
        'l4_selected_route_id': state.l4_path_selection.get('route_id'),
        'l4_path_selection_reason': state.l4_path_selection.get('reason'),
        'deterministic_available': state.deterministic_path.get('available'),
        'deterministic_decision': state.deterministic_path.get('decision'),
        'deterministic_retry_rule': state.deterministic_path.get('retry_rule'),
        'deterministic_ready_wall_clock_s': state.deterministic_path.get('ready_wall_clock_s'),
        ...
        'primary_class': state.primary.get('failure_class'),
        'selected_observation_line': state.selected_observation.get('line'),
        'selected_observation_class': state.selected_observation.get('failure_class'),
        'l0_deterministic_primary_line': state.l0_deterministic_primary.get('line'),
        'l0_deterministic_primary_class': state.l0_deterministic_primary.get('failure_class'),
        ...
        'l1_semantic_primary_class': state.l1_semantic_primary.get('failure_class'),
        'l2_grounded_primary_line': state.l2_grounded_primary.get('line'),
        'l2_grounded_primary_class': state.l2_grounded_primary.get('failure_class'),
        'l0_selected_observation_line': state.l0_selected_observation.get('line'),
        'l0_selected_observation_class': state.l0_selected_observation.get('failure_class'),
        'l1_selected_observation_line': state.l1_selected_observation.get('line'),
        'l1_selected_observation_class': state.l1_selected_observation.get('failure_class'),
        'l2_selected_observation_line': state.l2_selected_observation.get('line'),
        'l2_selected_observation_class': state.l2_selected_observation.get('failure_class'),
        'l1_observation_relation_to_l0': state.observation_selection.get('l1_relation_to_l0'),
        'l2_observation_relation_to_l0': state.observation_selection.get('l2_relation_to_l0'),
...
def _history_track_status(history: Mapping[str, Any]) -> str:
    if not history:
        return 'not_available'
    if history.get('available') is True:
        return 'available'
    return str(history.get('availability_reason') or 'not_available')
...
def _panel_row_gold_fields(state: _PanelRowState) -> dict[str, Any]:
    return {
        ...
        'gold_l0a_selected_observation_accuracy': state.gold_l0a.get(
            'selected_observation_accuracy'
        ),
        'gold_l0a_observation_fingerprint_accuracy': state.gold_l0a.get(
            'observation_fingerprint_accuracy'
        ),
        'gold_l0a_primary_absence_correct': state.gold_l0a.get('primary_absence_correct'),
        ...
        'gold_l1_observation_selection_correct': state.gold_l1.get('observation_selection_correct'),
        ...
        'gold_l2_identity_kind_correct': state.gold_l2.get('identity_kind_correct'),
        'gold_l2_expected_identity_kind': state.gold_l2.get('expected_identity_kind'),
        ...
        'gold_l2_primary_grounding_effect': state.gold_l2.get('primary_grounding_effect'),
        ...
        'gold_deterministic_action_correct': state.gold_deterministic_l4.get('action_correct'),
        'gold_deterministic_policy_action': state.gold_deterministic_l4.get('policy_action_pass'),
...
def _panel_row_l0_fields(state: _PanelRowState) -> dict[str, Any]:
    l0_identity_kind = state.l0_bundle_kpis.get('identity_kind') or (
        'root'
        if state.l0_bundle_kpis.get('root_fingerprint')
        else 'observation_only' if state.l0_bundle_kpis.get('observation_fingerprint') else 'none'
    )
    return {
        ...
        'l0_identity_kind': l0_identity_kind,
        'l0_selected_observation_line': state.l0_bundle_kpis.get('selected_observation_line'),
        'l0_selected_observation_class': state.l0_bundle_kpis.get('selected_observation_class'),
        'l0_observation_fingerprint': state.l0_bundle_kpis.get('observation_fingerprint'),
        'l0_observation_fingerprint_source': state.l0_bundle_kpis.get(
            'observation_fingerprint_source'
        ),
        'l0_affected_entity': state.l0_bundle_kpis.get('affected_entity'),
        'l0_affected_entity_fingerprint': _dict(state.l0_bundle_kpis.get('affected_entity')).get(
            'fingerprint'
        ),
        ...
        'l0_later_progress_after_fault_observations': state.l0_bundle_kpis.get(
            'later_progress_after_fault_observations'
        )
        or state.summary.get('later_progress_after_fault_observations')
        or [],
...
def _panel_row_l2_l3_l4_fields(state: _PanelRowState) -> dict[str, Any]:
    general_ceiling = _dict(state.l4_kpis.get('general_root_ceiling'))
    selected_ledger = _dict(state.l4_kpis.get('selected_policy_ledger'))
    effective_policy = _dict(state.l4_kpis.get('effective_policy'))
    no_progress_guard = _dict(state.l4_kpis.get('job_no_progress_guard'))
    unknown_guard = _dict(state.l4_kpis.get('job_unknown_progress_guard'))
    l2_identity_kind = state.l2_kpis.get('identity_kind') or (
        'root'
        if state.l2_kpis.get('root_fingerprint')
        else 'observation_only' if state.l2_kpis.get('observation_fingerprint') else 'none'
    )
    current_identity_kind = state.current_failure_facts.get('identity_kind') or (
        'root'
        if state.current_failure_facts.get('root_fingerprint')
        else (
            'observation_only'
            if state.current_failure_facts.get('observation_fingerprint')
            else 'none'
        )
    )
    return {
        ...
        'l2_selected_observation_available': state.l2_kpis.get('selected_observation_available'),
        'l2_recovery_assessment_audited': state.l2_kpis.get('recovery_assessment_audited'),
        ...
        'l2_observational_finding_count': state.l2_kpis.get('observational_finding_count'),
        ...
        'l2_identity_kind': l2_identity_kind,
        ...
        'l2_observation_fingerprint': state.l2_kpis.get('observation_fingerprint'),
        'l2_observation_fingerprint_source': state.l2_kpis.get('observation_fingerprint_source'),
        'l2_observation_fingerprint_available': state.l2_kpis.get(
            'observation_fingerprint_available'
        ),
        'l2_affected_entity': state.l2_kpis.get('affected_entity'),
        'l2_affected_entity_fingerprint': _dict(state.l2_kpis.get('affected_entity')).get(
            'fingerprint'
        ),
        ...
        'current_identity_kind': current_identity_kind,
        ...
        'current_observation_fingerprint': state.current_failure_facts.get(
            'observation_fingerprint'
        ),
        'current_observation_fingerprint_source': state.current_failure_facts.get(
            'observation_fingerprint_source'
        ),
        'current_affected_entity': state.current_failure_facts.get('affected_entity'),
        'current_affected_entity_fingerprint': _dict(
            state.current_failure_facts.get('affected_entity')
        ).get('fingerprint'),
        'l3_wall_clock_s': state.l3_kpis.get('wall_clock_s'),
        'l3_history_available': state.l3_kpis.get('history_available'),
        'l3_job_history_available': state.l3_kpis.get('job_history_available'),
        'l3_same_job_attempts': state.l3_kpis.get('same_job_attempts'),
        'l3_consecutive_same_job_no_advance_attempts': state.l3_kpis.get(
            'consecutive_same_job_no_advance_attempts'
        ),
        'l3_consecutive_same_job_unknown_progress_attempts': state.l3_kpis.get(
            'consecutive_same_job_unknown_progress_attempts'
        ),
        'l3_job_progress_advanced': state.l3_kpis.get('job_progress_advanced'),
        'l3_matching_root_attempts': state.l3_kpis.get('matching_root_attempts'),
        'l3_observation_history_available': state.l3_kpis.get('observation_history_available'),
        'l3_matching_observation_attempts': state.l3_kpis.get('matching_observation_attempts'),
        'l3_consecutive_same_observation_no_advance_attempts': state.l3_kpis.get(
        ...
        'l3_same_entity_attempts': state.l3_kpis.get('same_entity_attempts'),
        'l3_different_entity_attempts': state.l3_kpis.get('different_entity_attempts'),
        'l3_unknown_entity_attempts': state.l3_kpis.get('unknown_entity_attempts'),
        'l3_consecutive_same_root_no_advance_attempts': state.l3_kpis.get(
            'consecutive_same_root_no_advance_attempts'
        ),
        'l3_consecutive_same_root_and_entity_no_advance_attempts': state.l3_kpis.get(
        ...
        'l3_advanced_beyond_all_same_entity_comparable_attempts': state.l3_kpis.get(
        ...
        'l4_evidence_source': state.l4_kpis.get('evidence_source'),
        'l4_model_contribution': state.l4_kpis.get('model_contribution'),
        'l4_policy_version': state.l4_kpis.get('policy_version'),
        'l4_base_rule': state.l4_kpis.get('base_rule'),
        'l4_effective_rule': effective_policy.get('rule'),
        'l4_effective_policy_source': effective_policy.get('source'),
        'l4_effective_allowed_retries': effective_policy.get('allowed_retries'),
        'l4_retry_budget_exhausted': state.l4_kpis.get('retry_budget_exhausted'),
        'l4_exhausted_by': state.l4_kpis.get('exhausted_by') or [],
        'l4_general_root_applicable': general_ceiling.get('applicable'),
        'l4_general_root_allowed_retries': general_ceiling.get('allowed_retries'),
        'l4_general_root_matching_prior_attempts': general_ceiling.get('matching_prior_attempts'),
        'l4_general_root_observed_advance': general_ceiling.get('observed_advance'),
        'l4_general_root_exhausted': general_ceiling.get('exhausted'),
        'l4_general_root_inapplicable_reason': general_ceiling.get('inapplicable_reason'),
        'l4_selected_policy_ledger_present': bool(selected_ledger),
        'l4_selected_policy_ledger_rule': selected_ledger.get('rule'),
        'l4_selected_policy_ledger_scope': selected_ledger.get('history_match_scope'),
        'l4_selected_policy_allowed_retries': selected_ledger.get('allowed_retries'),
        'l4_selected_policy_matching_prior_attempts': selected_ledger.get(
            'matching_prior_attempts'
        ),
        'l4_selected_policy_observed_advance': selected_ledger.get('observed_advance'),
        'l4_selected_policy_exhausted': selected_ledger.get('exhausted'),
        'l4_job_no_progress_matching_prior_attempts': no_progress_guard.get(
            'matching_prior_attempts'
        ),
        'l4_job_no_progress_allowed_retries': no_progress_guard.get('allowed_retries'),
        'l4_job_no_progress_exhausted': no_progress_guard.get('exhausted'),
        'l4_job_unknown_matching_prior_attempts': unknown_guard.get('matching_prior_attempts'),
        'l4_job_unknown_allowed_retries': unknown_guard.get('allowed_retries'),
        'l4_job_unknown_exhausted': unknown_guard.get('exhausted'),
        ...
        'l4_current_affected_entity': state.l4_kpis.get('current_affected_entity'),
    ...
    if row.get("l1_response_parsed") is False:
        concerns.append(
            _concern(
                ...
                f"deterministic analyzer result via {row.get('decision_basis')}",
            )
        )
    ...
    if row.get("semantic_safety") == "stable_identity_adjusted":
        concerns.append(
            _concern(
                ...
                f"base_rule={row.get('l4_base_rule')} "
                f"effective_rule={row.get('l4_effective_rule')}",
            )
        )
...
def _default_l0_kpis(
    *,
    ...
    return {
        ...
        "later_progress_after_fault_observations": later_progress_after_fault_observations,
