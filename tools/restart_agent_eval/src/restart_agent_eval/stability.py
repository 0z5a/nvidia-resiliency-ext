...
def _sample_from_row(
    ...
    l1_assessment = _l1_assessment(analysis_result, l1_trace)
    assessment = l1_assessment.get("model_recovery_assessment") or {}
    if not isinstance(assessment, Mapping):
        assessment = {}
    policy_fields, policy_contract = _policy_fields(assessment)
    primary = l1_assessment.get("primary_failure") or analysis_result.get("primary_failure") or {}
    root_cause = l1_assessment.get("root_cause_assessment") or {}
    ...
    analysis_status = l1_assessment.get("analysis_status") or analysis_result.get("analysis_status")
    ...
    return {
        ...
        "failure_domain_confidence": _calibration_confidence(
            analysis_status,
            assessment,
            "failure_domain",
        ),
        "retry_outlook_confidence": _calibration_confidence(
            analysis_status,
            assessment,
            "retry_outlook_without_workload_change",
        ),
        "root_cause_status": root_cause.get("status"),
        "primary_class": primary.get("failure_class") or row.get("l1_semantic_primary_class"),
        "primary_line": primary.get("line") or row.get("l1_semantic_primary_line"),
        "identity_kind": row.get("current_identity_kind") or row.get("l2_identity_kind"),
        "root_fingerprint": row.get("current_root_fingerprint") or row.get("l2_root_fingerprint"),
        "observation_fingerprint": row.get("current_observation_fingerprint")
        or row.get("l2_observation_fingerprint"),
        "selected_observation_class": row.get("selected_observation_class")
        or row.get("l2_selected_observation_class"),
        "selected_observation_line": row.get("selected_observation_line")
        or row.get("l2_selected_observation_line"),
...
def _l1_assessment(
    ...
    """Prefer the public L1 block while allowing existing run artifacts."""

    value = analysis_result.get("l1_assessment") or l1_trace.get("semantic_payload")
    if isinstance(value, Mapping):
        return value
    legacy_root = analysis_result.get("root_cause_assessment")
    legacy_recovery = analysis_result.get("model_recovery_assessment")
    if isinstance(legacy_root, Mapping) or isinstance(legacy_recovery, Mapping):
        return {
            "root_cause_assessment": legacy_root or {},
            "model_recovery_assessment": legacy_recovery or {},
            "analysis_status": analysis_result.get("analysis_status"),
            "primary_failure": analysis_result.get("primary_failure"),
        }
    return {}
...
def _calibration_confidence(
    ...
    claim = assessment.get(claim_name) or {}
    if analysis_status != "primary_identified" or not isinstance(claim, Mapping):
        return None
    ...
    if isinstance(confidence, bool) or not isinstance(confidence, int):
        return None
    return confidence if 1 <= confidence <= 99 else None
...
def _summarize_cohort(
    ...
    return {
        ...
        "primary_and_identity_stability": {
            ...
            "identity_kind": _categorical_metrics(
                [sample.get("identity_kind") for sample in usable]
            ),
            "root_fingerprint": _categorical_metrics(
                [
                    sample.get("root_fingerprint")
                    for sample in usable
                    if sample.get("identity_kind") == "root"
                ]
            ),
            "selected_observation_class": _categorical_metrics(
                [
                    sample.get("selected_observation_class")
                    for sample in usable
                    if sample.get("identity_kind") == "observation_only"
                ]
            ),
            "selected_observation_line": _categorical_metrics(
                [
                    sample.get("selected_observation_line")
                    for sample in usable
                    if sample.get("identity_kind") == "observation_only"
                ]
            ),
            "observation_fingerprint": _categorical_metrics(
                [
                    sample.get("observation_fingerprint")
                    for sample in usable
                    if sample.get("identity_kind") == "observation_only"
                ]
            ),
...
def _render_cohort(cohort: Mapping[str, Any]) -> list[str]:
    ...
    for label, key in (
        ...
        ("selected observation class", "selected_observation_class"),
        ("selected observation line", "selected_observation_line"),
