...
from .panel_format import _claim_confidence_label
from .retry_policy_view import ledger_ratio
...
def write_review_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    selected_observation = summary.get("selected_observed_failure") or {}
    l4_kpis = summary.get("l4_kpis") or {}
    general_ceiling = l4_kpis.get("general_root_ceiling") or {}
    selected_ledger = l4_kpis.get("selected_policy_ledger") or {}
    effective = l4_kpis.get("effective_policy") or {}
    no_progress_guard = l4_kpis.get("job_no_progress_guard") or {}
    unknown_guard = l4_kpis.get("job_unknown_progress_guard") or {}
    ...
    semantic_comparison = summary.get("semantic_comparison") or {}
    recovery_assessment = {
        "failure_domain": semantic_comparison.get("failure_domain") or {},
        "retry_outlook_without_workload_change": (
            semantic_comparison.get("retry_outlook_without_workload_change") or {}
        ),
    }
    root_cause_status = semantic_comparison.get("root_cause_status")
    ...
    lines = [
        ...
        (
            ...
            f"base_rule=`{l4_kpis.get('base_rule')}`, "
            f"effective_rule=`{effective.get('rule')}`, "
            f"exhausted=`{l4_kpis.get('retry_budget_exhausted')}`, "
            f"exhausted_by=`{l4_kpis.get('exhausted_by') or []}`, "
            f"affected_entity=`{_entity_label(l4_kpis.get('current_affected_entity'))}`"
        ),
        (
            "- retry_ledgers: "
            f"general_root=`{ledger_ratio(general_ceiling)}` "
            f"exhausted=`{general_ceiling.get('exhausted')}`, "
            f"selected=`{ledger_ratio(selected_ledger)}`, "
            f"selected_scope=`{selected_ledger.get('history_match_scope')}`, "
            f"job_no_progress=`{ledger_ratio(no_progress_guard)}`, "
            f"job_unknown_progress=`{ledger_ratio(unknown_guard)}`"
        ),
        (
            "- primary: "
            f"`{primary.get('failure_class')}` line `{primary.get('line')}` "
            f"outcome `{primary.get('fault_outcome')}` "
            f"causal_role `{primary.get('causal_role')}`"
        ),
        (
            "- selected observation: "
            f"`{selected_observation.get('failure_class')}` "
            f"line `{selected_observation.get('line')}` "
            f"causal_role `{selected_observation.get('causal_role')}` "
            "(non-causal, root-independent)"
        ),
        (
            ...
            "domain_confidence="
            f"`{_claim_confidence_label(_claim_field(recovery_assessment, 'failure_domain', 'value'), _claim_field(recovery_assessment, 'failure_domain', 'status'), _claim_field(recovery_assessment, 'failure_domain', 'confidence'))}`, "
            "retry_outlook_without_workload_change="
            ...
            "retry_confidence="
            f"`{_claim_confidence_label(_claim_field(recovery_assessment, 'retry_outlook_without_workload_change', 'value'), _claim_field(recovery_assessment, 'retry_outlook_without_workload_change', 'status'), _claim_field(recovery_assessment, 'retry_outlook_without_workload_change', 'confidence'))}`"
        ),
    ...
    lines.extend(_affected_entity_markdown(primary.get("affected_entity")))
    ...
    lines.extend(_failure_tracks_markdown(summary))
...
def _artifact_guide_markdown(summary: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        "## Artifact Guide",
        "",
        "- Complete L1 assessment, L2 grounding, and final policy result: "
        f"`{artifacts.get('result_json')}`",
        f"- Deep pipeline trace, raw model response, and tool transcript: "
        f"`{artifacts.get('trace_json')}`",
        "- `review.json` stores harness evaluation and compact comparison fields; "
        "it does not duplicate the complete L1 or L2 product payloads.",
    ]


def _affected_entity_markdown(value: Any) -> list[str]:
    if not isinstance(value, dict):
        return []
    return [
        "",
        "## Grounded Affected Entity",
        "",
        f"- kind: `{value.get('kind')}`",
        f"- identity: `{value.get('identity')}`",
        f"- fingerprint: `{value.get('fingerprint')}`",
        f"- evidence_line: `{value.get('evidence_line')}`",
    ]
...
def _gold_score_markdown(gold_score: Any) -> list[str]:
    ...
    deterministic_l4 = gold_score.get("deterministic_l4") or {}
    ...
    lines.extend(
        [
            ...
            (
                ...
                f"primary_grounding_effect=`{l2.get('primary_grounding_effect')}`"
            ),
            (
                "- L4/final product: "
            ...
            (
                "- deterministic versus enriched action: "
                f"deterministic=`{deterministic_l4.get('action_correct')}`, "
                f"enriched=`{enriched_l4.get('action_correct')}`, "
                f"effect=`{path_comparison.get('action_effect')}`"
            ),
            (
                "- deterministic versus enriched policy/action: "
                f"deterministic=`{deterministic_l4.get('policy_action_pass')}`, "
...
def _l2_kpis_markdown(l2_kpis: dict[str, Any]) -> list[str]:
    return [
        ...
        (
            ...
            f"identity_kind=`{l2_kpis.get('identity_kind')}`, "
        ...
        (
            ...
            f"recovery_assessment_audited=`{l2_kpis.get('recovery_assessment_audited')}`, "
            f"influence=`{l2_kpis.get('audit_influence')}`, "
        ...
        (
            ...
            f"available=`{l2_kpis.get('observation_fingerprint_available')}`, "
            f"selected=`{l2_kpis.get('selected_observation_available')}`, "
            f"source=`{l2_kpis.get('observation_fingerprint_source')}`, "
            f"id=`{l2_kpis.get('observation_fingerprint')}`"
        ),
        (
            "- findings: "
            f"wall_clock_s=`{l2_kpis.get('wall_clock_s')}`, "
            f"count=`{l2_kpis.get('finding_count')}`, "
            f"observational=`{l2_kpis.get('observational_finding_count')}`, "
...
def _l3_kpis_markdown(l3_kpis: dict[str, Any]) -> list[str]:
    return [
        ...
        (
            ...
            f"identity_kind=`{l3_kpis.get('identity_kind')}`, "
            f"history_identity_ready=`{l3_kpis.get('history_identity_ready')}`, "
            f"root=`{l3_kpis.get('current_root_fingerprint')}`, "
            f"observation=`{l3_kpis.get('current_observation_fingerprint')}`, "
            f"entity=`{_entity_label(l3_kpis.get('current_affected_entity'))}`"
        ),
        (
            "- history comparison: "
        ...
        (
            ...
            f"same/different/unknown_entity="
            f"`{l3_kpis.get('same_entity_attempts')}/"
            f"{l3_kpis.get('different_entity_attempts')}/"
            f"{l3_kpis.get('unknown_entity_attempts')}`, "
            f"root_streak=`{l3_kpis.get('consecutive_same_root_no_advance_attempts')}`, "
            "root_entity_streak="
            f"`{l3_kpis.get('consecutive_same_root_and_entity_no_advance_attempts')}`, "
            "observation_streak="
            f"`{l3_kpis.get('consecutive_same_observation_no_advance_attempts')}`"
        ),
        ("- timing: " f"wall_clock_s=`{l3_kpis.get('wall_clock_s')}`"),
    ]


def _failure_tracks_markdown(summary: dict[str, Any]) -> list[str]:
    tracks = summary.get("failure_tracks") or {}
    history = summary.get("l3_history") or {}
    selection = summary.get("l4_path_selection") or {}
    route_id = selection.get("route_id") or summary.get("target")
    route_history = (history.get("routes") or {}).get(route_id) or {}
    deterministic = tracks.get("deterministic") or {}
    primary = tracks.get("primary") or {}
    observation = tracks.get("observation") or {}
    return [
        "",
        ...
        "| track | state | history |",
        "|---|---|---|",
        f"| deterministic | `{deterministic.get('status') or 'not_available'}` | "
        f"`{_history_status(history.get('deterministic'))}` |",
        f"| primary | `{primary.get('status') or 'not_available'}` | "
        f"`{_history_status(route_history.get('primary'))}` |",
        f"| observation | `{observation.get('status') or 'not_available'}` | "
        f"`{_history_status(route_history.get('observation'))}` |",
        "",
        f"- selected path: `{selection.get('path')}`",
        f"- selected route: `{selection.get('route_id')}`",
...
def _history_status(value: Any) -> str:
    ...
    if value.get("available") is True:
        return "available"
    return str(value.get("availability_reason") or "not_available")
...
def _l0_bundle_kpis_markdown(l0_bundle_kpis: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        (
            ...
            f"observation_line=`{l0_bundle_kpis.get('selected_observation_line')}`, "
            f"identity_kind=`{l0_bundle_kpis.get('identity_kind')}`, "
        ...
        (
            "- observation fingerprint: "
            f"available=`{l0_bundle_kpis.get('observation_fingerprint_available')}`, "
            f"source=`{l0_bundle_kpis.get('observation_fingerprint_source')}`, "
            f"id=`{l0_bundle_kpis.get('observation_fingerprint')}`"
        ),
        (
            "- progress_context: "
...
def _model_selection_signals_markdown(signals: dict[str, Any]) -> list[str]:
    lines = [
        ...
        (
            ...
            f"retry_base_rule=`{signals.get('retry_policy_base_rule')}`, "
...
def _model_call_reliability_markdown(model_call_summary: dict[str, Any]) -> list[str]:
    lines = [
        ...
        (
            ...
            f"timeout_kinds=`{model_call_summary.get('timeout_kind_counts')}`"
        ),
        (
            "- latency: "
    ...
    if provider_errors:
        lines.extend(
            [
                "",
                "| turn | attempt | status | timeout kind | retryable | retry scheduled | latency s | error |",
                "|---:|---:|---:|---|---|---|---:|---|",
            ]
        )
...
def _semantic_safety_markdown(semantic_safety: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        (
            ...
            f"retry_base_rule=`{semantic_safety.get('retry_policy_base_rule')}`, "
