...
def test_incomplete_l1_contract_is_rejected_without_manufactured_evidence(tmp_path):
    ...
    assert payload["result_provenance"]["model_contribution"] == (
        "attempted_not_used_contract_invalid"
    )
...
def test_l1_parser_distinguishes_contract_errors_from_syntax_errors():
    evidence = _current_evidence({})
    ...
    assert error.value.contract_errors == ("top-level has unsupported fields: decision",)
...
def test_l1_final_contract_invalid_output_retains_payload_and_validation_errors(tmp_path):
    ...
    extractor.complete_evidence["decision"] = "STOP"
    bundle = build_l0_bundle(str(log_path))
    ...
    assert result.semantic_payload["decision"] == "STOP"
    assert result.errors == ("top-level has unsupported fields: decision",)
...
def test_l1_prohibited_action_field_is_rejected_by_closed_contract(tmp_path):
    ...
    assert analyzer.last_trace["layers"]["L1"]["output_status"] == "contract_invalid"
    assert analyzer.last_trace["layers"]["L2"]["grounding_status"] == "not_run"
    assert analyzer.last_trace["l2_audit"]["not_run_reason"] == "l1_output_unusable"
