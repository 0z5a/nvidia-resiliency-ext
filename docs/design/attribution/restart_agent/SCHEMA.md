...
identifies the exact observed object
only for `identity_kind=root`:
...
| Output | `L0ModelFacingView` | Bounded initial L1 evidence portion plus trace-only projection metrics. |
...
`L0ModelFacingView.prompt_payload()` contains `decision_evidence`,
`attempt_execution_context`, and `evidence_bundle`. L1 owns the complete
machine-readable user-message envelope and adds its generated
`response_schema` to those three sections. The L0B `schema_version` and
`projection_metrics` are not model-visible. Transporting the response schema
in the L1 envelope does not make it dynamic evidence. Immutable restart
guarantees remain in the static system prompt and are not repeated here.
...
array limits, confidence bounds, evidence support tags, minimum
primary/root-cause citation requirements, and non-primary semantics. Recovery
support completeness is evaluated by L2 so an omitted recovery tag cannot make
...
| --- | --- | --- | --- | --- | --- |
| `primary_identified` | Required initiating or unknown-role primary | Optional non-primary surfaces; selected id may reference one canonical terminal surface | Substantive assessment of the primary | Assesses the primary | Relationships to the primary are allowed |
...
| `insufficient_evidence` | `null` | Zero or more grounded candidates; selected id references exactly one item or is `null` when selection is ambiguous | Unknown initiating cause with one or more explicit evidence gaps | May contain substantive current-attempt claims only when a selected observation is grounded; otherwise both claims are `unknown` | Empty because no relationship to a primary can be established |
...
Item ids are unique. Causal role is `unknown`, `cascade`, or `teardown`;
`initiating` would require `primary_failure`. Every item cites one or more
canonical evidence ids. `selected_observed_failure_id` is either null or
references exactly one item. It is required when the model can select one
...
line. Evidence IDs are unique and `supports` uses only the closed claim tags
...
| Input | `model_view` | `L0ModelFacingView` | Exact initial model-facing evidence portion and Decision Evidence. |
...
`l1_assessment` is the exact parsed `restart_agent_evidence.v1` object emitted by
the model. L2 does not rewrite, normalize, or replace fields inside this block.
It includes the model's primary selection, root-cause assessment, recovery
assessment, related failures, evidence, and rationales exactly as L1 returned
them after response-contract parsing.
