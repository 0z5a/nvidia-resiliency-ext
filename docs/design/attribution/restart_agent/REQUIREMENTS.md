...
- **RA-05: Deterministic evidence.** Before any model call, L0A MUST construct
  ...
  a null primary and a separate observation-only identity. L0B MUST create a bounded,
...
- **RA-10: Declared policy context.** Policy information supplied outside the
  ...
  and progress history only consume its retry budget.
  **Verify:** context-present, context-absent,
  changed-iteration, observer-mismatch, and policy-override tests.
