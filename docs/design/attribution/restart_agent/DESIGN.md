...
- `L0B.md`: deterministic context management, the bounded attention-efficient
  Initial Model Evidence View, selection accounting, and L0B KPIs.
...
- Trusted policy context supplied outside the log belongs in L4. The active
  ...
  terminal CUDA OOM. The active `rejected_iteration_retry_then_skip` context
...
restart_agent_request.v1 + restart_agent_config.v1 + PriorAttemptView
  ...
  -> AttemptRecord(progress + deterministic failure facts)
       ...
       +-> L0B context management -> Initial Model Evidence View
...
| L0B | `L0Bundle + DecisionEvidence -> L0ModelFacingView` | Context management through bounded attention projection and selection/lossiness accounting. See `L0B.md`. |
