...
require ``--no-requeue`` because their compact decision is keyed by array-task
ID.
...
The consumer derives ``segment_health_check.<job_id>.state`` in the configured
directory. ``job_id`` is ``SLURM_ARRAY_JOB_ID`` for an array or ``SLURM_JOB_ID``
for a regular job. The first record is a compact array of excluded allocation
IDs: array-task IDs for an array, or the regular job ID.

.. code-block:: json

   ["7","12"]

The first record must use this exact byte format: a newline-terminated compact
array of quoted decimal allocation IDs without whitespace. CRLF, spaces,
integer IDs, and object-shaped records are invalid. The consumer matches an
exact allocation ID, so task ``7`` does not match task ``17``, and reads no
records after the first line.

Missing, malformed, oversized, or unreadable artifacts fail open. A matching
decision marks the current allocation unit unhealthy through the existing segment
health-check path.
...
A malformed or unreadable decision emits a warning. An exclusion is reported
by the existing health-check error path. Other launchers do not install the
segment check or read the artifact.
