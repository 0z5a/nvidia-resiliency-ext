...
def _validate_grep_log(
    ...
    _reject_extra_arguments(arguments, {"pattern", "ignore_case", "max_matches"})
...
L1_TOOL_CONTRACTS: dict[str, L1ToolContract] = {
    ...
    "grep_log": L1ToolContract(
        name="grep_log",
        description="Search the bound log with a Python regular expression.",
        argument_schema={
            "type": "object",
            "properties": {
                "pattern": {
        ...
        result_required_fields=("pattern", "matches", "total_matches", "truncated"),
