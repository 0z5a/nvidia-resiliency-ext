...
- Workload code, data, configuration, and workload-selected software remain unchanged.
  Failed process state is recreated, and normal restart delay applies. Physical nodes or
  devices may be replaced, but replacement preserves the hardware and software BOM,
  resource capacity and limits, and storage access. The allocation resource envelope is
  invariant across the homogeneous pool.
