...
def _root_occurrence_groups(
    ...
    if primary is None:
        return ()
    referenced_ids = set(selected.references.get("occurrence_group_ids") or ())
    groups = tuple(
        group
        for group in bundle.occurrence_groups
        if group.occurrence_group_id in referenced_ids
    ...
    return tuple(
        group
        for group in bundle.occurrence_groups
        if selected.identity_line in {group.first_line, *group.sample_lines}
    )
